...
def load_comments(uploaded_file) -> pd.DataFrame:
    ...
    text_aliases = ["text", "comment", "comments", "feedback", "review", "message", "body", "content"]
    text_col = next((c for c in df.columns if c in text_aliases), None)
    if text_col is None and df.shape[1] == 1:
        text_col = df.columns[0]
    if text_col is None:
        raise ValueError("Uploaded file must contain a text column (e.g. 'text', 'comment', 'feedback', 'review').")
